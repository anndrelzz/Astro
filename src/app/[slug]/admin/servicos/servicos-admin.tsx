...
// Lista a esquerda, painel de edicao a direita: selecionar um servico abre o
// formulario ao lado, sem trocar de tela.
...
export function ServicosAdmin({ servicosIniciais }: { servicosIniciais: Servico[] }) {
  ...
  const editando = criando || selecionado !== null;
  ...
  async function salvar(e: React.FormEvent) {
    ...
    const url = criando ? "/api/servicos" : `/api/servicos/${selecionado}`;
  ...
  async function remover() {
    ...
    const resposta = await fetch(`/api/servicos/${selecionado}`, {
      method: "DELETE",
    });
    ...
    <div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_26rem]">
            ...
            <table className="w-full min-w-[44rem] text-sm">
              ...
              <tbody>
                {servicosIniciais.map((s) => {
                  const aberto = selecionado === s.id;
                  return (
                    <tr
                      key={s.id}
                      onClick={() => abrirServico(s)}
                      className={`cursor-pointer border-b border-admin-border/60 transition last:border-0 ${
                        aberto
                          ? "bg-astro-blue/10 ring-1 ring-inset ring-astro-blue-bright/40"
                          : "hover:bg-white/[0.03]"
                      } ${s.ativo ? "" : "opacity-50"}`}
                    >
        ...
        {editando ? (
          <form
            onSubmit={salvar}
            className="flex flex-col gap-4 rounded-2xl border border-admin-border bg-admin-surface p-5"
          >
            <div>
              <p className="astro-label">
                {criando ? "Novo servico" : "Editar servico"}
              </p>
              <h2 className="mt-1 text-lg font-semibold text-white">
                {rascunho.nome || "Sem nome"}
              </h2>
            </div>
              ...
              <div className="mt-3 space-y-3">
                {SEGMENTOS.map((seg) => (
                  <div key={seg.chave} className="flex items-center gap-3">
                    <span className="w-14 shrink-0 text-sm text-slate-200">
                      {seg.rotulo}
                    </span>
                    <input
                      type="range"
                    ...
                    <div className="flex w-24 shrink-0 items-center gap-1 rounded-lg border border-admin-border bg-admin-bg px-2 py-1.5">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setRascunho({
                            ...rascunho,
                            [seg.chave]: Number(e.target.value),
                          })
                        }
          ...
          <div className="hidden items-center justify-center rounded-2xl border border-dashed border-admin-border p-10 text-center xl:flex">
            <div>
              <p className="text-sm font-semibold text-white">Nenhum servico aberto</p>
              <p className="mt-1 text-sm text-astro-muted">
                Clique num servico da lista para editar, ou crie um novo.
              </p>
            </div>
          </div>
        )}
